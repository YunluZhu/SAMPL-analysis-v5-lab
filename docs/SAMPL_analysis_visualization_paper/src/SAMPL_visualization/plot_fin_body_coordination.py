...
def plot_fin_body_coordination(root, **kwargs):
    ...
    body rotation is calculated as rotation from initial to 40ms before time of peak speed
    ...
    print('------\n+ Plotting atk angle and fin-body ratio (rotation to -40ms)')
    ...
    if if_multiple_repeats:
        ...
        coef_filename = os.path.join(fig_dir,f"fin-body coordination modeling coef (repeats)(rot to -40ms).csv")
        ...
        filename = os.path.join(fig_dir,"attack angle vs rotation (repeats)(rot to -40ms).pdf")
        ...
        filename = os.path.join(fig_dir,"attack angle vs rotation(rot to -40ms).pdf")
        plt.savefig(filename,format='PDF')
        plt.close()
        ...
        coef_filename = os.path.join(fig_dir,f"fin-body coordination modeling coef(rot to -40ms).csv")
    ...
    filename = os.path.join(fig_dir,"Fin-body ratio(rot to -40ms).pdf")
    plt.savefig(filename,format='PDF')
    plt.close()
    ...
    filename = os.path.join(fig_dir,"Sigmoid height(rot to -40ms).pdf")
    plt.savefig(filename,format='PDF')
    plt.close()
# %%
