...
NOTE:
It is recommneded to fit into a full parabola with 3 free parameters. Then use the mean x_intersect to make half parabola.
The x intersect should be within (-10,10)
...
NOTE: bounds in parabola_fit_centered() confines the upper and lower limites of the parameters. These may need to be changed according to your own data sets.
...
# root = "/Users/yunluzhu/Lab/Lab2/Data/VF/SAMPL_data/combined_7DD_data"
...
def distribution_binned_average(df, bin_width, condition):
    ...
    df = df.sort_values(by='propBoutIEI_pitch')
    df = df.assign(y_boutFreq = 1/df['propBoutIEI'])
    bins = pd.cut(df['propBoutIEI_pitch'], list(np.arange(0,90,bin_width)))
    grp = df.groupby(bins)
...
# %%
# Main function
def sensitivity_jacknife_halfP(root):
    ...
    for condition_idx, folder in enumerate(folder_paths):
        ...
        for subpath, subdir_list, subfile_list in os.walk(folder):
            ...
            if subdir_list:
                all_day_angles = pd.DataFrame()  # reset
    ...
    for i, age in enumerate(age_condition):
        ...
        # Calculate p values for sensitivity
