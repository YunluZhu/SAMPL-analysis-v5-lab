...
folder_name = f'{pick_data}_PCA_vis_feat'
...
ALIGNED_BOUT_RANGE = range(20,36)
        ...
        if subdir_list:
            ...
            for expNum, exp in enumerate(subdir_list):
                ...
                re_format = angles_grp.apply(
                    lambda x: pd.concat([
                                        x.iloc[ALIGNED_BOUT_RANGE,0].rename(lambda i: 'heading_'+str("%.2d" % i)),
                                        x.iloc[ALIGNED_BOUT_RANGE,1].rename(lambda i: 'pitch_'+str("%.2d" % i)),
                                        x.iloc[ALIGNED_BOUT_RANGE,2].rename(lambda i: 'speed_'+str("%.2d" % i)),
                                        ])
                )
                
                re_format = pd.concat([re_format, 
                                       angles_grp.propBoutAligned_time.head(1).reset_index().propBoutAligned_time,
                                       peak_angles.loc[:,'traj'].reset_index(drop=True),
                                       ],axis=1)
                ...
                # long_format_data = pd.concat(long_format_data,angles)
...
# %% sanity check
data=data_to_ana.sample(n=5)

fig, axs = plt.subplots(nrows=4, figsize = (30,25))

tmp = data.loc[:,(data.columns).str.contains('heading')]
tmp.sort_index(axis=1, inplace=True)
sns.lineplot(data=tmp.T,dashes=False,ax=axs[0],
             legend=False)

tmp = data.loc[:,(data.columns).str.contains('pitch')]
tmp.sort_index(axis=1, inplace=True)
sns.lineplot(data=tmp.T,dashes=False,ax=axs[1],
             legend=False)

tmp = data.loc[:,(data.columns).str.contains('speed')]
tmp.sort_index(axis=1, inplace=True)
sns.lineplot(data=tmp.T,dashes=False,ax=axs[2],
             legend=False)

tmp = data.loc[:,(data.columns).str.contains('heading')].values - data.loc[:,(data.columns).str.contains('pitch')].values
sns.lineplot(data=tmp.T,dashes=False,ax=axs[3],
             legend=False)

fig.savefig(fig_dir+f"/sample_HPS{str(datetime.now())}.pdf",format='PDF')
fig.tight_layout
