...
from plot_functions.plt_tools import (set_font_type, defaultPlotting, jackknife_list, jackknife_mean)
...
# %%
which_data = 'hc4' # all or specific data
...
# %%
def main(pick_data,frame_rate):
    FRAME_RATE = frame_rate
    
    idxRANGE = [15,46]
    TSP_THRESHOLD = [-np.Inf,-50,50,np.Inf]
    spd_bins = np.arange(5,22,2)
    ...
    # %%
    # CONSTANTS
    total_aligned = int(5/4 * FRAME_RATE)+1
    peak_idx = int(3/4 * FRAME_RATE)
    ...
    # sns.kdeplot(x='rot_l_accel', y='rot_l_decel',hue='speed_bins',data=all_feature_cond.sample(n=5000))
...
if __name__ == '__main__':
    ...
    if which_data == 'all':
        ...
        for pick_data in tqdm(all_data):
            ...
            main(pick_data,FRAME_RATE)
    else:
        print(f"Plotting {which_data} data:")
        main(which_data,FRAME_RATE)        
