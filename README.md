...
1. `grab_fish_angle` updated to fix a bug during bout segmentation and generation of `spd_bout_window`. The bug is raised when an epoch contains no detectable bout. The bug is rare and if you have not encountered `ValueError("The number of bouts windows doesn't match the number of speed windows.")` during analysis, there's no need to re-analyze the dataset. 
