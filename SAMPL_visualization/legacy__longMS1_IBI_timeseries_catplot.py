...
after_1s_mask = df_valid["time_relative_s"] > 0.5
...
#%%
df_filtered_linearPhase = df_filtered[df_filtered.time_relative_s < 0.25].copy()

df_acceleration_attributes = df_filtered_linearPhase.groupby('unique_IBI_idx', observed=True).agg(
    angVel_accel_avg=('angVelSmoothed', lambda x: x.diff().median()*FRAME_RATE),
    yvel_accel_avg=('yvel', lambda x: x.diff().median()*FRAME_RATE),
    cond0=('cond0', 'first'),
    cond1=('cond1', 'first'),
    expNum=('expNum', 'first'),
...
#% plot angVel_accel_avg yvel_accel_avg
median_data = df_acceleration_attributes.groupby(['cond0','cond1','expNum'], observed=True)[['angVel_accel_avg', 'yvel_accel_avg']].median().reset_index()

#%%
data_to_plot = median_data.query("cond1 == 'ld'")
for param in ['angVel_accel_avg', 'yvel_accel_avg']:
    ...
    plt.savefig(os.path.join(fig_dir,f"IBI linear phase {param} by cond0_cond1.pdf"), format='PDF')
    
    print(f"\n--- ANOVA for {param} ---")
    df_param = data_to_plot.dropna(subset=[param])
...
df_tomodel_all_mod['expNum'] = 'rep_combined'
...
# draw fitted line from df_fit_raw parameters
time_fit = np.linspace(0, 3, 100)
for var in df_fit_raw['variable'].unique():
    g = sns.FacetGrid(df_fit_raw.query("variable == @var"), row="cond1", col="cond0", hue='expNum', sharex=True, sharey='row')
    def plot_fit(data, color, **kwargs):
        for _, row in data.iterrows():
            y_fit = exp_decay(time_fit, row['v_terminal'], row['tau'], row['v0'])
            plt.plot(time_fit, y_fit, color=color, **kwargs)
