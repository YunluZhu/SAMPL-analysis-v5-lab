    ...
    for subpath, subdir_list, subfile_list in os.walk(folder):
        ...
        if subdir_list:
            ...
            jack_y_all = pd.concat([jack_y_all, jack_y.assign(age=all_conditions[condition_idx].split("_")[0], 
                                                            cond1=all_conditions[condition_idx].split("_")[1])], axis=0, ignore_index=True)
            ...
            ang_std_all = pd.concat([ang_std_all, ang_std.assign(age=all_conditions[condition_idx].split("_")[0], cond1=all_conditions[condition_idx].split("_")[1])], axis=0, ignore_index=True)
...
age_cond1 = list(jack_y_all['cond0'].values)
age_condition.sort()
age_cond1 = set(age_condition)
...
for i, age in enumerate(age_condition):
    ...
    g = sns.lineplot(
        x='Posture (deg)',y='Probability', hue='cond1', 
        style='cond0', data=fitted, ci='sd', err_style='band', 
