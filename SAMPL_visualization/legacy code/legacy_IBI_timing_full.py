...
#     df_out = df.groupby(np.arange(len(df))//samples_per_bin)[['propBoutIEI_pitch','y_boutFreq']].mean().assign(dpf=condition[0],cond1=condition[4:])
#     return df_out
...
jackknifed_coef.columns = ['sensitivity','x_inter','y_inter','cond0','cond1','jackknife_excluded_sample']
...
coef_ori.columns = ['sensitivity','x_inter','y_inter','cond0','cond1']
...
for i, age in enumerate(age_condition):
    ...
    g = sns.lineplot(x='x',y='y',hue='cond1',data=fitted, ci="sd", ax=axes[i]).set(title=age)
    g = sns.scatterplot(x='propBoutIEI_pitch',y='y_boutFreq',hue='cond1',s=20, data=binned, alpha=0.3, ax=axes[i],linewidth=0)
    g.set_xticks(np.arange(-40,60,20))      # adjust ticks
    ...
    # X INTERSECT
    sns.swarmplot(x='cond1', y='x_inter', data=coef_plt,ax=axes[i+age_cond_num*2])
    # Y INTERSECT
    sns.swarmplot(x='cond1', y='y_inter', data=coef_plt,ax=axes[i+age_cond_num*3])
    
plt.savefig(fig_dir+f"/{pick_data}'s sensitivity_3full.pdf",format='PDF')
