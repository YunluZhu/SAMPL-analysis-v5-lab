...
root = "/Volumes/LabData/SAMPL_FA_PCLesion/YZ_code"
...
def sigfunc_2free(x, a, b, c, d):
    ...
    return   -1.702909e-08 + (7.734157+1.702909e-08)/(1 + np.exp(-(a*(x + b)))) 
...
atk_max_other = 7.734157  # also paste d value here for atk_angle calculation
# %%
jackknifed_coef = pd.DataFrame()  # coef results calculated with jackknifed pitch data
jackknifed_y = pd.DataFrame()  # fitted y using jackknifed pitch data
...
jackknifed_coef.sort_values(by=['cond1','cond0','excluded_exp'],inplace=True, ignore_index=True)            
jackknifed_y.reset_index(inplace=True)
# %% 
# PLOTs
...
hue_order=['1ctrl','2activation']  # you may personalize the order of conditions
