...
fig_dir = "/Users/yunluzhu/Documents/Lab2/Data/SAMPL_ana/Figures/NT_hets_tmp_plots"
        ...
        if subdir_list:
            ...
            for expNum, exp in enumerate(subdir_list):
                ...
                output_forBout = pd.DataFrame(data={'atk_ang':atk_ang.values, 
                                             ...
                                             'time':peak_angles_day.time.values,
...
tsne = TSNE(n_components=2, verbose=1, perplexity=100,n_iter=1000,early_exaggeration=15,learning_rate=max(30000/15/4,200))
...
# get_clusters = DBSCAN(eps=1.2, min_samples=10).fit_predict(tsne_results) # parameters for 7dd tau Night
get_clusters = DBSCAN(eps=2, min_samples=20).fit_predict(tsne_results) # parameters for 7dd tau Day, with perplexity=100
# get_clusters = DBSCAN(eps=1.5, min_samples=20).fit_predict(tsne_results) 
...
# cluster.tsneplot(score=tsne_results, colorlist = get_clusters, 
#     legendpos='upper right', legendanchor=(1.15, 1))



# %% visualize
# choose the groups to visualize. see all_conditions for conditions
# plot control on the left and cond on the right
is_ctrl = 0
is_cond = 1

res_toplt = pd.DataFrame(data = tsne_results,
                         columns=['TSNE1', 'TSNE2'])
...
figure.savefig(fig_dir+"/_newfig.pdf",format='PDF')
...
#     alpha=0.05,
# )
# figure2.savefig(fig_dir+"/_cond.pdf",format='PDF')
# %%
