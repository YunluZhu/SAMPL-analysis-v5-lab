...
boot_hazard_df['hazard_smooth'] = boot_hazard_df.groupby(['boot','cond0','ztime','ang0_bin'], observed=True)['hazard'].transform(
...
# Main loop
results = []
for (boot, cond0, ztime), group_df in boot_hazard_df.groupby(
    ['boot', 'cond0', 'ztime'], observed=True
):
...
def compute_deviance_explained(results):
    ...
    pd.DataFrame with columns:
        boot, cond0, ztime, deviance_explained
    """
    rows = []
    for r in results:
        model = r['model']
...
def compute_delta_deviance_explained(results, boot_hazard_df, model_vars):
    ...
    for r in results:
        boot, cond0, ztime = r['boot'], r['cond0'], r['ztime']

        df = boot_hazard_df.query(
            "boot == @boot and cond0 == @cond0 and ztime == @ztime"
        ).copy()
...
def plot_partial_dependence_bootstrap(cond0, ztime, results, boot_hazard_df, model_vars, fig_dir, save_individual=True):
    """
    ...
    sub = [r for r in results if r['cond0']==cond0 and r['ztime']==ztime]
    if not sub:
        print(f"No results for {cond0} | {ztime}")
        return

    df_cond = boot_hazard_df[(boot_hazard_df['cond0']==cond0) & (boot_hazard_df['ztime']==ztime)]
    ...
    if save_individual:
        plt.savefig(os.path.join(fig_dir, f'partial_dependence_bootstrap_{cond0}_{ztime}.pdf'), dpi=300, bbox_inches='tight')
        plt.show()
...
for cond0, ztime in sorted(unique_conditions):
    plot_partial_dependence_bootstrap(
        cond0=cond0,
        ztime=ztime,
        results=results,
        boot_hazard_df=boot_hazard_df,
