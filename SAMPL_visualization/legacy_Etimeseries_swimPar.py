...
epoch_data_ctrl, epoch_info_ctrl = extract_epochs('/Volumes/LabDataPro/manuscript data/2022-11 depths exploration/behavior data/otog/DD_hets')
epoch_data_cond, epoch_info_cond = extract_epochs('/Volumes/LabDataPro/manuscript data/2022-11 depths exploration/behavior data/otog/DD_otog')

epoch_data_all = pd.concat([epoch_data_ctrl.assign(cond1='ctrl'), epoch_data_cond.assign(cond1='cond')], ignore_index=True)
# %%

avg_byepoch = epoch_data_all.groupby(['cond1', 'epochNum'])['pitch'].median().reset_index()
