...
def extract_consecutive_bout_features(connected_bout_df:pd.DataFrame, list_of_features:list, max_lag:int):
    ...
    connected_bout_df = connected_bout_df.assign(
        epoch_conduid = connected_bout_df['cond0'] + connected_bout_df['cond1'] + connected_bout_df['ztime'] + connected_bout_df['expNum'].astype(str) + connected_bout_df['epoch_uid'],
        exp_conduid = connected_bout_df['cond0'] + connected_bout_df['cond1'] + connected_bout_df['ztime'] + connected_bout_df['expNum'].astype(str),
    )
    connected_bout_df = connected_bout_df.groupby(['epoch_conduid'],group_keys=False).filter(lambda g: len(g)>max_lag)
    ...
    for feature_toplt in list_of_features:
        ...
        grouped = connected_bout_df.groupby(['cond1','cond0','ztime','expNum'],group_keys=False)
        col_selected = feature_toplt
        for (cond1, cond0, ztime, expNum), group in grouped:
            shift_df = pd.concat([group[col_selected].shift(-i).rename(f'{col_selected}_{i}') for i in range(max_lag+1)], axis=1)
            this_df_tocorr = shift_df.groupby(group['epoch_conduid'],group_keys=False).apply(
        ...
        sel_bouts = df_tocorr
        if 'post_IBI' not in feature_toplt:
            sel_bouts = sel_bouts.loc[sel_bouts[f'{feature_toplt}_{max_lag}'].notna()]
        else:
            sel_bouts = sel_bouts.loc[sel_bouts[f'{feature_toplt}_{max_lag-1}'].notna()]
        ...
        long_df = long_df.sort_values(by=['cond0','cond1','ztime','expNum','id','lag'])
        if consecutive_bout_features.empty:
            consecutive_bout_features = long_df
        else:
            consecutive_bout_features = consecutive_bout_features.merge(long_df, on=['id', 'lag','cond1','cond0','ztime','expNum','exp_conduid'], how='left')
...
def cal_autocorrelation_feature(this_cond_df:pd.DataFrame, col_selected:str, col_groupby:str, max_lag:int):
    ...
    if ("epoch_conduid" not in this_cond_df.columns):
        this_cond_df = this_cond_df.assign(
            epoch_conduid = this_cond_df['cond0'] + this_cond_df['cond1'] + this_cond_df['expNum'].astype(str) + this_cond_df['epoch_uid'],
    ...
    for j in np.arange(1, max_lag+1):
        ...
        if len(this_df[this_df[f'{col_selected}_{j}'].notna()]) >= 10:
            ...
            # for plotting / long form
            this_df.columns = ['ori', 'shifted']
            long_form_shifted = pd.concat(
                [long_form_shifted, this_df.assign(lag=j)],
                ignore_index=True
            )
