...
def get_aligned_bouts_wIBI(root:str, FRAME_RATE:int, **kwargs):
    ...
    for condition_idx, folder in enumerate(folder_paths):
        ...
        for subpath, subdir_list, subfile_list in os.walk(folder):
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    for i in day_night_split(bout_time,'aligned_time',ztime=which_zeitgeber).index:
        ...
        around_peak_data = pd.concat(around_peak_data_, ignore_index=True).assign(cond0=cond0,cond1=cond1)
        all_around_peak_data_.append(around_peak_data)
        
        bout_features = pd.concat(bout_features_, ignore_index=True).assign(
            cond0=cond0,
            cond1=cond1
            )
        all_feature_cond_.append(bout_features)
        
    all_around_peak_data = pd.concat(all_around_peak_data_, ignore_index=True)
    all_around_peak_data = all_around_peak_data.assign(time_ms = (all_around_peak_data['idx']-peak_idx)/FRAME_RATE*1000)
    
    all_feature_cond = pd.concat(all_feature_cond_, ignore_index=True)
...
def extract_bout_features_v5(bout_data:pd.DataFrame,peak_idx:int, FRAME_RATE:int, **kwargs):
    ...
    idx_initial_phase = np.arange(idx_initial,idx_prep_200)
    ...
    this_exp_features = pd.DataFrame(data={
        ...
        'traj_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase),:].groupby('bout_num')['propBoutAligned_instHeading'].mean().values, 
        'spd_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase),:].groupby('bout_num')['propBoutAligned_speed'].mean().values, 
...
def get_bout_features(root:str, FRAME_RATE:int, **kwargs):
    ...
    for condition_idx, folder in enumerate(folder_paths):
        ...
        all_cond0.append(cond0)
        all_cond1.append(cond1)
        bout_features = pd.concat(bout_features_, ignore_index=True)
        bout_features = bout_features.assign(
            cond0=cond0,
            cond1=cond1
            )
        all_feature_cond_.append(bout_features)
