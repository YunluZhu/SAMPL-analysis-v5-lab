"""
Updated 251111 for faster loading
"""
...
def compute_fwhm_per_bout(df, FRAME_RATE:int):
    ...
    half_speed_local = peak_speed_local / 2.0
    ...
    for i in range(peak_idx_local, 0, -1):
        if speeds[i] >= half_speed_local and speeds[i-1] < half_speed_local:
            left_time = times[i]
            break


    for i in range(peak_idx_local, len(speeds)-1):
        if speeds[i] >= half_speed_local and speeds[i+1] < half_speed_local:
            right_time = times[i]
            break

    fwhm_local = right_time - left_time if pd.notna(left_time) and pd.notna(right_time) else np.nan

    return pd.Series({
        'asc_time_new': left_time,
        'desc_time_new': right_time,
        'WHM_new': fwhm_local/FRAME_RATE,
        'peak_speed_new': peak_speed_local,
        'half_speed_new': half_speed_local
    })

def get_aligned_bouts_wIBI(root:str, FRAME_RATE:int, ztime:str='day', if_strict_DayNightSplit:bool=False, if_get_bout_features:bool=True, **kwargs):
        ...
        for subpath, subdir_list, subfile_list in os.walk(folder):
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    for i in day_night_split(bout_time,'aligned_time',ztime=which_zeitgeber,narrow_bin=if_strict_DayNightSplit).index:
                    ...
                    around_peak_data_.append(exp_data)
                    
                    if if_get_bout_features:
                        ###################### get connected bouts
                        ...
                        IBI_swim_after[if_between_epochs] = None
                        ...
                        this_exp_features = this_exp_features.assign(
                            ...
                            post_IBI = IBI_swim_after,
                            pre_IBI = IBI_swim_before,
                            cond0 = cond0,
                            cond1 = cond1,
                        )
                        # day night split. also assign ztime column
                        this_ztime_exp_features = day_night_split(this_exp_features,'bout_time',ztime=which_zeitgeber,narrow_bin=if_strict_DayNightSplit)
    ...
    all_around_peak_data = pd.concat(around_peak_data_, ignore_index=True)
    
    if if_get_bout_features:
        all_feature_cond = pd.concat(bout_features_, ignore_index=True)
    ...
    all_cond1.sort()
    return all_around_peak_data, all_feature_cond, all_cond0, all_cond1, idxRANGE
...
def extract_bout_features_v5(bout_data:pd.DataFrame,peak_idx:int, FRAME_RATE:int, **kwargs):
    ...
    idx_full_aligned = np.arange(idx_initial,idx_end)
    ...
    this_exp_features = pd.DataFrame(data={
        ...
        'spd_initial':bout_data.loc[bout_data['idx']==idx_initial,'propBoutAligned_speed'].values, 
        'spd_peak':bout_data.loc[bout_data['idx']==peak_idx,'propBoutAligned_speed'].values, 
        'angvel_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase),:].groupby('bout_num')['propBoutAligned_angVel'].median().values, 
        'angvel_prep_phase': bout_data.loc[bout_data['idx'].isin(idx_prep_phase),:].groupby('bout_num')['propBoutAligned_angVel'].median().values, 
        'angvel_post_phase': bout_data.loc[bout_data['idx'].isin(idx_post_phase),:].groupby('bout_num')['propBoutAligned_angVel'].median().values, 
        ...
        'traj_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase),:].groupby('bout_num')['propBoutAligned_instHeading'].median().values, 
        'spd_initial_phase': bout_data.loc[bout_data['idx'].isin(idx_initial_phase),:].groupby('bout_num')['propBoutAligned_speed'].median().values, 
        
        'fish_length': bout_data.loc[bout_data['idx'].isin(idx_full_aligned),:].groupby('bout_num')['fish_length'].mean().values, 
        
    })
    
    if 'boxNum' in bout_data.columns:
        this_exp_features = this_exp_features.assign(
            boxNum = bout_data.loc[bout_data['idx']==peak_idx,'boxNum'].values,
        )
    ...
    y_pre_swim = bout_data.loc[swim_indicator].groupby('bout_num').head(1)['propBoutAligned_y'].values
    y_post_swim = bout_data.loc[swim_indicator].groupby('bout_num').tail(1)['propBoutAligned_y'].values
    y_swim = y_post_swim - y_pre_swim
    
    x_pre_swim = bout_data.loc[swim_indicator].groupby('bout_num').head(1)['propBoutAligned_x'].values
    x_post_swim = bout_data.loc[swim_indicator].groupby('bout_num').tail(1)['propBoutAligned_x'].values
    x_swim = x_post_swim - x_pre_swim
    
    # addedd 2026 June Jiahuan Liu's function
    fwhm_result = bout_data.groupby('bout_num').apply(compute_fwhm_per_bout, FRAME_RATE=FRAME_RATE)
    ...
    this_exp_features = this_exp_features.assign(rot_total=this_exp_features['pitch_end']-this_exp_features['pitch_initial'],
                                                ...
                                                depth_chg = yy,
                                                depth_chg_fullBout = yfull,
                                                x_chg = absxx,
                                                x_chg_fullBout = absfullx,
                                                ...
                                                WHM=fwhm_result['WHM_new'].values,
                                                half_spd_peak = fwhm_result['half_speed_new'].values,
                                           
                                                )  
    return this_exp_features


def get_bout_features(root:str, FRAME_RATE:int, ztime:str='day', if_strict_DayNightSplit:bool=False, **kwargs):
    """
    extract bout features from aligned bout data (bout_data.h5) for all conditions
    args:
    root: str
    FRAME_RATE: int, frame rate of the data, used to calculate the index of different time points in a bout
    ztime: str, 'day' or 'night', used for day night split. default is 'day'
    if_strict_DayNightSplit: bool, whether to use strict day night split (day: 10 to 22, night: 0 to 8) or not (day: 9 to 23, night: 23 to 9). default is False (not strict split)
    """
    peak_idx , total_aligned = get_index(FRAME_RATE)
    T_start = -0.3
    T_end = 0.25
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    if not max_angvel_df.empty:
                        max_angvel_time = max_angvel_df.query("cond0 == @cond0 and cond1 == @cond1")['max_angvel_time'].item()
...
def get_max_angvel_rot(root, FRAME_RATE,**kwargs):
    # to be modified for faster speed
    peak_idx , total_aligned = get_index(FRAME_RATE)
...
def get_connected_bouts(root:str, FRAME_RATE:int, **kwargs):
    """
    fixed the unique epoch ID issue
    """
    ...
    for key, value in kwargs.items():
        if key == 'ztime':
            ...
            max_angvel_df = value
        elif key == 'if_strict_DayNightSplit':
            if_narrow_bin = value
    # %%
    # CONSTANTS
    BIN_NUM = 4  # number of speed bins
        ...
        for subpath, subdir_list, subfile_list in os.walk(folder):
            ...
            if subdir_list:
                ...
                for expNum, exp in enumerate(subdir_list):
                    ...
                    # day night split. also assign ztime column
                    this_ztime_exp_features = day_night_split(this_exp_features,'bout_time',narrow_bin=if_narrow_bin, ztime=which_zeitgeber)
