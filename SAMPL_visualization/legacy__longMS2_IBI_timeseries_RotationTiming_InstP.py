...
# %%
data_to_plot = IBI_features.query("frequency < 10 and ztime=='night'")
...
MIN_COUNT_PER_BIN = 100 # minimum number of data points required per bin
...
# DURATION_THRESH = np.percentile(tv_df['duration'], 90) # seconds; only include IBIs shorter than this

# -----------------------------
# 1. Filter by duration threshold , already done above
# -----------------------------
tv_df_short = tv_df#.loc[tv_df['duration'] <= DURATION_THRESH].copy()
...
POSTURE_STEP = snap_step(15 * fd_ang, [0.5, 1, 1.5, 2, 3, 5])
...
a_min_data = tv_df_short["ang"].min()
a_max_data = tv_df_short["ang"].max()
...
tv_df_short["a_bin"] = pd.cut(tv_df_short["ang"], bins=a_edges, include_lowest=True, labels=a_bin_mid)
# -----------------------------
...
# first ang of each IBI
data_to_plot = data_to_plot.assign(
    first_ang = data_to_plot.groupby('unique_IBI_idx')['ang'].transform('first')
)
...
data_to_plot = data_to_plot.assign(
    posture_groups = pd.cut(
        data_to_plot['first_ang'],
        bins=reduced_a_bins,
        include_lowest=True
    )
)
...
df_binned = (
    data_to_plot
    .assign(first_ang = data_to_plot.groupby('unique_IBI_idx')['ang'].transform('first'))
    .assign(
        posture_groups = pd.cut(
            data_to_plot['first_ang'],
            bins=reduced_a_bins,
    ...
    .groupby(['cond0','expNum','posture_groups'])
...
support = (
    df_binned.dropna()
    .groupby(['cond0','posture_groups','bin_center'])['expNum']
    .nunique()
    .reset_index(name='n_exp')
)
...
valid_bins = support.loc[support['frac_exp'] >= MIN_FRACTION_EXP, ['cond0','posture_groups','bin_center']]

# filter df_binned
df_binned_filtered = df_binned.merge(valid_bins, on=['cond0','posture_groups','bin_center'], how='inner')
#%%
...
df_binned_filtered_avg = (
    df_binned_filtered.groupby(['cond0','posture_groups','bin_center'])[f'binned_{what_y}']
    .mean()
    .reset_index()
)
for c, Z_smooth_df in heatmaps_smooth.items():
    ...
    for pg, group in df_lines.groupby('posture_groups'):
        ax.plot(
            group['bin_center'].values,
            group[f'binned_{what_y}'].values,
            lw=1.2,
            alpha=0.5,
            color='black'
        )
