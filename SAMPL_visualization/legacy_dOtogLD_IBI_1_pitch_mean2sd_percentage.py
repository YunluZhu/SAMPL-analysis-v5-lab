...
# for feature in ['IBIpitch_mean', 'IBIpitch_sd','IBIpitch_mean2sd']:
#     x_name = 'cond1'
#     gridrow = 'ztime'
#     gridcol = 'cond0'
#     units = 'expNum'
...
#         aspect = 0.8,
#         )
#     filename = os.path.join(fig_dir,f"{prename}{feature}_{x_name}__{gridcol}X{gridrow}.pdf")
#     plt.savefig(filename,format='PDF')
#     plt.show()
...
for feature in ['IBIpitch_std_percent']:
    ...
    plt.show()
# %%
IBI_toplt_percent = pd.DataFrame()
for expNum, group in IBI_toplt.groupby('expNum'):
    cond_df = group.query("cond1 == 'otog' and cond0 == 'DD'")['IBIpitch_sd'].values
    ctrl_df = group.query("cond1 == 'hets' and cond0 == 'LD'")['IBIpitch_sd'].values
    percent = (cond_df - ctrl_df)/ctrl_df
    try:
        IBI_toplt_percent = pd.concat([IBI_toplt_percent, pd.DataFrame(data={'IBIpitch_std_percent':percent,
                                                                             'expNum':expNum,
                                                                             'cond1': 'DDotog-LDhets'})], ignore_index=True)
    except:
        continue
