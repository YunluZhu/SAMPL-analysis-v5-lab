...
pick_data = 'tau_bkg' # name of your dataset to plot as defined in function get_data_dir()
...
if DAY_RESAMPLE+NIGHT_RESAMPLE > 0:
    folder_name = f'IBI1_pitchPerct_z{which_ztime}_resample_zD{DAY_RESAMPLE}_zN{NIGHT_RESAMPLE}'
else:
    folder_name = f'IBI1_pitchPerct_z{which_ztime}'
...
if if_jackknife:
    if which_ztime != 'night':
        ...
        for (this_cond1, this_cond0, this_ztime), group in IBI_angles_day_resampled.groupby(cat_cols):
            ...
            for excluded_exp, idx_group in enumerate(index_matrix):
                this_median = group.loc[group['expNum'].isin(idx_group),['IBI_pitch']].median().to_frame(name='IBIpitch_median')
                this_75 = np.percentile(group.loc[group['expNum'].isin(idx_group),'IBI_pitch'], 75)
                this_25 = np.percentile(group.loc[group['expNum'].isin(idx_group),'IBI_pitch'], 25)
                resampled_day_std = pd.concat([resampled_day_std, this_median.assign(cond0=this_cond0,
                                                                        cond1=this_cond1,
                                                                        expNum=excluded_exp,
                                                                        ztime=this_ztime,
                                                                        IBIpitch_75=this_75,
    ...
    if which_ztime != 'day':
        ...
        for (this_cond1, this_cond0, this_ztime), group in IBI_angles_night_resampled.groupby(cat_cols):
            ...
            for excluded_exp, idx_group in enumerate(index_matrix):
                ...
                this_75 = np.percentile(group.loc[group['expNum'].isin(idx_group),'IBI_pitch'], 75)
                this_25 = np.percentile(group.loc[group['expNum'].isin(idx_group),'IBI_pitch'], 25)
                resampled_night_std = pd.concat([resampled_night_std, this_median.assign(cond0=this_cond0,
                                                                        cond1=this_cond1,
                                                                        expNum=excluded_exp,
                                                                        ztime=this_ztime,
                                                                        IBIpitch_75=this_75,
    ...
    IBI_toplt = IBI_toplt.assign(IBIpitch_median = IBI_toplt['IBI_pitch'])
    IBI_toplt = IBI_toplt.assign(IBIpitch_75 = IBI_angles_cond.groupby(['ztime','cond0','cond1','exp','expNum'])['IBI_pitch'].apply(lambda g: np.percentile(g, 75)).values,
                                 IBIpitch_25 = IBI_angles_cond.groupby(['ztime','cond0','cond1','exp','expNum'])['IBI_pitch'].apply(lambda g: np.percentile(g, 25)).values,
                                 )
IBI_toplt = IBI_toplt.assign(
    quartile_wid = IBI_toplt['IBIpitch_75'] - IBI_toplt['IBIpitch_25']
)
...
for feature in ['IBIpitch_median', 'IBIpitch_75','quartile_wid']:
    x_name = 'cond1'
    gridrow = 'ztime'
    gridcol = 'cond0'
    units = 'expNum'
...
# # %%
# IBI_toplt = IBI_toplt.assign(
#     mean2sd = IBI_toplt['mean_val'] + 2* IBI_toplt['IBIpitch_std']
# )
...
# condition2_df = IBI_toplt.loc[IBI_toplt['cond1'] == all_cond1[2],feature]
# condition_df = IBI_toplt.loc[IBI_toplt['cond1'] == all_cond1[1],feature]
# control_df = IBI_toplt.loc[IBI_toplt['cond1'] == all_cond1[0],feature]
