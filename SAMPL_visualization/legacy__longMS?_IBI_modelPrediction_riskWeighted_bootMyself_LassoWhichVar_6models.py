...
if len(aic_df) == 0:
    ...
    n_boots_per_condition = aic_df.groupby(['cond0', 'ztime']).size().reset_index(name='n_boots')
    stability_counts = stability_counts.merge(n_boots_per_condition, on=['cond0', 'ztime'])
    stability_counts['frequency'] = stability_counts['selection_count'] / stability_counts['n_boots']
...
all_variables = coef_df['variable'].unique()
...
for time_period in ['day', 'night']:
    ...
    coef_time = coef_df[coef_df['ztime'] == time_period].copy()
...
coef_df, boot_hazard_df, aic_df, predictions_list, fig_dir
...
for time_idx, time_period in enumerate(['day', 'night']):
    ...
    aic_time = aic_df[aic_df['ztime'] == time_period].copy()
