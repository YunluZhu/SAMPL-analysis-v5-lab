...
# 5. Remove any ang0_bins that became empty after filtering (optional but safe)
...
# hazard_df_filtered['n_event_smooth'] = hazard_df_filtered['hazard_smooth'] * hazard_df_filtered['n_risk']

# Optional: add a small pseudo-count to avoid exact 0 or 1
EPS = 1e-3
# hazard_df_filtered['n_event_smooth'] = np.clip(
#     hazard_df_filtered['n_event_smooth'], EPS, hazard_df_filtered['n_risk'] - EPS
# )
...
for (cond0, ztime), df in hazard_df_filtered.groupby(['cond0','ztime'], observed=True):
    df = df.copy()
...
# -----------------------------
a_grid = np.sort(hazard_df_filtered['ang0_bin'].astype(float).unique())
t_grid = np.sort(hazard_df_filtered['t_mid'].unique())
...
for r in results:
    ...
    pred_heatmaps_smooth_glm[(r['cond0'], r['ztime'])] = Z
...
sns.catplot(
    data=plot_df, 
    y='coef', x='cond0', row='term',
    col='ztime',
)
plt.savefig(os.path.join(fig_dir, 'glm_coefs_by_condition.pdf'))

#%%
# remove quadratic terms for simplicity
# -----------------------------
# Step 1: Fit GLM per cond0/ztime
# -----------------------------
results_linear = []
...
for (cond0, ztime), df in hazard_df_filtered.groupby(['cond0','ztime'], observed=True):
    ...
    # Design matrix (linear + interaction)
    X = sm.add_constant(df[['a_z','t_z','a_t']])
...
# -----------------------------
# Step 2: Build predicted heatmaps
# -----------------------------
a_grid = np.sort(hazard_df_filtered['ang0_bin'].astype(float).unique())
t_grid = np.sort(hazard_df_filtered['t_mid'].unique())
AA, TT = np.meshgrid(a_grid, t_grid, indexing='ij')

pred_heatmaps_linear = {}
...
for r in results_linear:
    ...
    Xp = sm.add_constant(np.column_stack([a_z, t_z, a_t]))

    # Predict probabilities
    p_hat = model.predict(Xp)
    Z = p_hat.reshape(AA.shape)
    pred_heatmaps_linear[(r['cond0'], r['ztime'])] = Z

# -----------------------------
# Step 3: Plot example
# -----------------------------

for condition, heatmap in pred_heatmaps_linear.items():
