...
from plot_functions.get_bout_features import (get_aligned_bouts_wIBI, get_bout_features, get_connected_bouts, extract_bout_features_v5)
from plot_functions.get_bout_kinetics import get_kinetics
from plot_functions.get_IBIangles import get_IBIangles
from plot_functions.plt_tools import (set_font_type, defaultPlotting,day_night_split)
from plot_functions.plt_functions import (plt_categorical_grid, plt_network_graphs)
from plot_functions.get_bout_correlation import get_cluster_featuresIBI
...
pick_data = 'depth_7d'
which_ztime = 'all'
compare_which = 'ztime' # condition for separation None for treat as whole
nCluster = 10
sort_by_feature='pitch_initial' # by which parameter to sort the clusters on the figure

root, FRAME_RATE = get_data_dir(pick_data)

folder_name = f'{pick_data} clusterFeatures_sim_by{compare_which}'
...
connected_bouts, all_cond0, all_cond1 = get_connected_bouts(root, FRAME_RATE, ztime=which_ztime)
# IBI_angles, cond0, cond1 = get_IBIangles(root, FRAME_RATE, ztime=which_ztime)

# %%
all_feature_clustered = get_cluster_featuresIBI(connected_bouts, nCluster)


# %%
connected_bout_features = connected_bouts[connected_bouts['post_IBI_time'].notna()]
# # connected_bout_features = connected_bout_features[connected_bout_features['pre_IBI_time'].notna()]
connected_bout_features = connected_bout_features.assign(
    cluster = all_feature_clustered['cluster'],
)
connected_bout_features = connected_bout_features.assign(
    epoch_uid = connected_bout_features['cond1'] + connected_bout_features['expNum'].astype(str) + connected_bout_features['epoch_uid'],
)
connected_bout_features = connected_bout_features.assign(
    bout_uid_adj = connected_bout_features['epoch_uid']+ '__' + connected_bout_features['bout_uid'],
    to_bout_uid_adj = connected_bout_features['epoch_uid']+ '__' + connected_bout_features['to_bout'],
)

to_cluster = connected_bout_features[['to_bout_uid_adj']].merge(connected_bout_features[['cluster','bout_uid_adj']], left_on='to_bout_uid_adj',right_on='bout_uid_adj',how='left')
...
for sel_condition in connected_bout_features[compare_which].unique():
    cond_data = connected_bout_features.loc[connected_bout_features[compare_which]==sel_condition,['cluster','to_cluster','expNum','cond0','cond1']]
    connected_bout_appearance = cond_data.groupby('cluster').size()
    graph_df = cond_data.groupby(['cluster','to_cluster']).size().reset_index()
    ...
    # %
    sel_cond_all_features = connected_bout_features
    if use_condition_par:
        sel_cond_all_features = connected_bout_features.loc[connected_bout_features[compare_which]==sel_condition]
    ...
    IBI_values = total_bout_grouped_byCluster['post_IBI_time']
    ...
    IBI_ychg_values = total_bout_grouped_byCluster['x_chg']
        
    total_bout_appearance = connected_bout_features.loc[connected_bout_features[compare_which]==sel_condition].groupby('cluster').size()
