...
# %%
data_list = ['PC_LD'] # all or specific data
...
# g = sns.relplot(
#     data=df_features_combined.sample(20000),
#     x=x_name,
#     y=y_name,
#     col='cond1',
#     row='dataset',
...
# )
# g.set(
#     xlim=[np.percentile(df_features_combined[x_name],0.1), np.percentile(df_features_combined[x_name],99.9)],
#     ylim=[np.percentile(df_features_combined[y_name],0.1), np.percentile(df_features_combined[y_name],99.9)],
# )  

# # %% determine attack angle thresholds
# x_name = 'atk_ang'
# hist_df = pd.DataFrame()
# x_range_min = np.percentile(df_features_combined[x_name],3)
# x_range_max = np.percentile(df_features_combined[x_name],97)
# x_range = np.arange(x_range_min, x_range_max, 2)

# for (dataset), this_df in df_features_combined.groupby(['dataset']):  
#     ctrl_data = this_df.query("cond1 == 'ctrl'")
#     cond_data = this_df.query("cond1 == 'cond'")
#     hist_ctrl, bin_edges = np.histogram(ctrl_data[x_name], bins=x_range, density=True)
#     hist_cond, bin_edges = np.histogram(cond_data[x_name], bins=x_range, density=True)
#     hist_output = hist_cond-hist_ctrl
#     this_hist_df = pd.DataFrame(data={'density': hist_output,
#                                  'bins': (bin_edges[1:]+bin_edges[:-1])/2,}).assign(
#                                      dataset = dataset,
#                                  )

#     hist_df = pd.concat([hist_df, this_hist_df], ignore_index=True)

# g = sns.relplot(data=hist_df, kind='line', x='bins', y='density', hue='dataset')
# plt.plot(x_range,np.zeros_like(x_range))
...
for (dataset, cond1), this_df_ori in df_toplt.groupby(['dataset','cond1']):  
...
#         hue='cond1',hue_order=['ctrl','cond'], dodge=False, sharey=False, height=3, aspect=0.6
#     )
...
for (row_val, col_val), ax in g.axes_dict.items():
    thiscond_data = df_toplt.query("dataset==@row_val & cond1==@col_val")
...
#                     hue='cond1', hue_order=['ctrl','cond'], stat='count',
#                     palette = 'gray'
#     )
#     ax.legend([],[], frameon=False)
...
for (dataset, cond1), this_df in df_toplt.groupby(['dataset','cond1']):  
    x_range_min = -30#np.percentile(this_df[x_name],0.5)
    x_range_max = 30 #np.percentile(this_df[x_name],99)
    this_df = this_df.loc[(this_df[x_name]>=x_range_min) & (this_df[x_name]<=x_range_max)]
