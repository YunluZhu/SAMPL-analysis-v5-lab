...
pick_data = (
    "creTau8"  # name of your dataset to plot as defined in function get_data_dir()
)
which_ztime = "day"  # 'day', 'night', or 'all'
DAY_RESAMPLE = 1000
NIGHT_RESAMPLE = (
    0  # Bouts drew from each experimental repeat (int.) 0 for no resampling
)
if_jackknife = True
if_use_maxAngvelTime_perCond1 = True  # if to calculate max adjusted angvel time for each condition and selectt range for body rotation differently
...
if if_use_maxAngvelTime_perCond1:
    max_angvel_time, all_cond0, all_cond1 = get_max_angvel_rot(
        root, FRAME_RATE, ztime=which_ztime
    )
    all_feature_cond, all_cond0, all_cond1 = get_bout_features(
        root, FRAME_RATE, ztime=which_ztime, max_angvel_time=max_angvel_time
    )
else:
    all_feature_cond, all_cond0, all_cond1 = get_bout_features(
        root, FRAME_RATE, ztime=which_ztime
...
# %% tidy data
all_feature_cond = all_feature_cond.sort_values(by=["cond1", "expNum"]).reset_index(
    drop=True
)
if FRAME_RATE > 100:
    df_toplt = all_feature_cond.drop(
        all_feature_cond.loc[
            (all_feature_cond["atk_ang"] < 0)
            & (
                all_feature_cond["rot_full_accel"]
                > all_feature_cond["rot_full_accel"].median()
            )
        ].index
    )
...
if which_ztime != "night":
    angles_day_resampled = df_toplt.loc[df_toplt["ztime"] == "day", :]
...
if which_ztime != "day":
    angles_night_resampled = df_toplt.loc[df_toplt["ztime"] == "night", :]
...
toplt = df_toplt.assign(
    traj_dir=pd.cut(
        df_toplt["traj_peak"], bins=[-90, 0, 90], labels=["negTraj", "posTraj"]
    )
)
...
binned_df = toplt.groupby(["cond1", "cond0"]).apply(
    lambda group: distribution_binned_average(
        group, by_col=x, bin_col=y, bin=AVERAGE_BIN
    )
)
...
for (cond1, cond0, cond_ztime), for_fit in df_toplt.groupby(
    ["cond1", "cond0", "ztime"]
):
    if if_jackknife:
    ...
    binned_df = distribution_binned_average(
        for_fit, by_col=which_rotation, bin_col=which_atk_ang, bin=AVERAGE_BIN
    )
    binned_df.columns = ["rot to angvel max", which_atk_ang]
    all_binned_average = pd.concat(
        [
            all_binned_average,
            binned_df.assign(
                cond0=cond0,
...
for i, g_row in enumerate(g.axes):
    for j, ax in enumerate(g_row):
        sns.lineplot(
            data=all_binned_average.loc[
                (all_binned_average["cond0"] == all_cond0[j])
                & (all_binned_average["ztime"] == all_ztime[i]),
                :,
            ],
            x="rot to angvel max",
            y=which_atk_ang,
