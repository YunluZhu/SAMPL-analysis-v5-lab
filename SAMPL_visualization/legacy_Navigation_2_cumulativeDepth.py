...
list_of_features = ['traj_peak', 'pitch_peak', 'spd_peak',
                    ...
                    'y_initial','y_end','displ_swim',
                    'xdispl_swim','post_IBI_align_time', 'pre_IBI_align_time',
                    ]
...
sel_consecutive_bouts = consecutive_bout_features.sort_values(by=['cond1','cond0','expNum','id','lag']).reset_index(drop=True)
# %%
pitch_bins = [-90,0,20, 90]

sel_consecutive_bouts = sel_consecutive_bouts.assign(
    cumu_swim_ydispl = sel_consecutive_bouts.groupby(['cond1','cond0','expNum','id'])['ydispl_swim'].apply(np.cumsum).values,
    pitch_peak_bins = pd.cut(sel_consecutive_bouts['pitch_peak_first'], bins=pitch_bins, labels=['dive','flat','climb']),
    traj_peak_bins = pd.cut(sel_consecutive_bouts['traj_peak_first'], bins=pitch_bins, labels=['dive','flat','climb']),
...
remove_last_bout = remove_last_bout.assign(
    cumu_ISIydispl = remove_last_bout.groupby(['cond1','cond0','expNum','id'])['IBI_swim_ydispl'].apply(np.cumsum).values,
    cumu_IBIydispl = remove_last_bout.groupby(['cond1','cond0','expNum','id'])['IBI_bout_ydispl'].apply(np.cumsum).values,
)
...
df_sum = sel_consecutive_bouts.groupby(['cond1','cond0','id','exp_conduid'])['ydispl_swim','xdispl_swim','displ_swim'].sum().reset_index()
df_sum = df_sum.assign(
    ...
    first_traj = sel_consecutive_bouts.groupby(['cond1','cond0','id','exp_conduid'])['traj_peak'].head(1).values,
    avg_spd = sel_consecutive_bouts.groupby(['cond1','cond0','id','exp_conduid'])['spd_peak'].mean().values,
)
...
df_sum['y_ratio'] = df_sum['ydispl_swim']/df_sum['displ_swim']
sns.lmplot(
    data = df_sum.query("displ_swim > 1"),
    x='first_traj',
    y='y_ratio',
    hue='cond1',
    scatter=False
)
plt.savefig(os.path.join(fig_dir, f"cumudisplYRatio-{name_of_x}.pdf"),format='PDF')
