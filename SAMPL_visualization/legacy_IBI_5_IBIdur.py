...
sel_features = ['IBI_pauseDur']
# Calculate std:
# std_jackknife v= IBI_avg_cond.groupby(cond_cols).apply(
...
if if_jackknife:
    if which_ztime != 'night':
        ...
        for (this_cond1, this_cond0, this_ztime), group in IBI_angles_day_resampled.groupby(cat_cols):
            ...
            for excluded_exp, idx_group in enumerate(index_matrix):
                this_std = group.loc[group['expNum'].isin(idx_group),sel_features].median().to_frame().T
                # this_mean = group.loc[group['expNum'].isin(idx_group),sel_features].mean().T
    ...
    if which_ztime != 'day':
        ...
        for (this_cond1, this_cond0, this_ztime), group in IBI_angles_night_resampled.groupby(cat_cols):
            ...
            for excluded_exp, idx_group in enumerate(index_matrix):
                this_std = group.loc[group['expNum'].isin(idx_group),sel_features].median().to_frame().T
                # this_mean = group.loc[group['expNum'].isin(idx_group),sel_features].mean()
    ...
    IBI_avg_cond = resampled_std
        
else:
    IBI_avg_cond = IBI_angles_cond.groupby(['ztime','cond0','cond1','exp','expNum']).median().reset_index()
...
# %%
for to_plot_feature in sel_features:
    ...
    filename = os.path.join(fig_dir,f"{prename}{to_plot_feature}avg__by{x_name}__{gridcol}X{gridrow}.pdf")
    plt.savefig(filename,format='PDF')
    plt.show()
