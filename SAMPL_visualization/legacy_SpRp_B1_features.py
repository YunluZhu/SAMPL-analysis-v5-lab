'''
Plot bout features - UP DOWN separated by pitch_initial = 10 deg
...
pick_data = 'tau_gfp_long' # name of your dataset to plot as defined in function get_data_dir()
which_ztime = 'day' # 'day', 'night', or 'all'
DAY_RESAMPLE = 0 # Bouts drew from each experimental repeat (int.) 0 for no resampling
if_jackknnife = False # Whether to jackknife (bool)
...
folder_name = f'SpRp_BF1_byPitchIni_z{which_ztime}_sample{DAY_RESAMPLE}'
...
df_sel = all_feature_cond.loc[
    (all_feature_cond['rot_full_accel']>0) & \
        (all_feature_cond['rot_full_decel']>0)
        ]
# %%
#mean
cat_cols = ['cond1','expNum','cond0','bout_time','ztime', 'exp']
feature_to_plt = [c for c in df_sel.columns if c not in cat_cols]
feature_for_comp = feature_to_plt + ['expNum']
# jackknife
all_feature_sampled = df_sel
...
for feature in feature_to_plt:
    ...
    filename = os.path.join(fig_dir,f"{prename}{feature}__by{x_name}__{gridcol}X.pdf")
