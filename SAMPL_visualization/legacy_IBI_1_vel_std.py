...
if DAY_RESAMPLE+NIGHT_RESAMPLE > 0:
    folder_name = f'IBI1_yvel_z{which_ztime}_resample_zD{DAY_RESAMPLE}_zN{NIGHT_RESAMPLE}'
else:
    folder_name = f'IBI1_yvel_z{which_ztime}'
...
IBI_data, cond0, cond1 = get_IBIangles(root, FRAME_RATE, ztime=which_ztime)
IBI_data_cond = IBI_data.loc[:,['propBoutIEI_yvel','ztime','expNum','cond0','cond1','exp']]
IBI_data_cond.columns = ['IBI_yvel','ztime','expNum','cond0','cond1','exp']
IBI_data_cond.reset_index(drop=True,inplace=True)
cond_cols = ['ztime','cond0','cond1']
all_ztime = list(set(IBI_data_cond.ztime))
all_ztime.sort()
...
# std_jackknife v= IBI_std_cond.groupby(cond_cols).apply(
    ...
    # )
    # std_jackknife = std_jackknife.loc[:,'IBI_yvel'].reset_index()
    
# %%
# sanity check
# cat_cols = ['cond0','cond1','ztime','exp']
# check_df = IBI_data.groupby(cat_cols).size().reset_index()
...
if if_jackknife:
    if which_ztime != 'night':
        IBI_data_day_resampled = IBI_data_cond.loc[
            IBI_data_cond['ztime']=='day',:
                ]
        if DAY_RESAMPLE != 0:  # if resampled
            IBI_data_day_resampled = IBI_data_day_resampled.groupby(
                    ['cond0','cond1','exp']
                    ).sample(
                            n=DAY_RESAMPLE,
                            replace=True
        ...
        for (this_cond1, this_cond0, this_ztime), group in IBI_data_day_resampled.groupby(cat_cols):
            ...
            for excluded_exp, idx_group in enumerate(index_matrix):
                this_std = group.loc[group['expNum'].isin(idx_group),['IBI_yvel']].std().to_frame(name='IBI_yvel_std')
                this_mean = group.loc[group['expNum'].isin(idx_group),['IBI_yvel']].mean()
    ...
    if which_ztime != 'day':
        IBI_data_night_resampled = IBI_data_cond.loc[
            IBI_data_cond['ztime']=='night',:
                ]
        if NIGHT_RESAMPLE != 0:  # if resampled
            IBI_data_night_resampled = IBI_data_night_resampled.groupby(
                    ['cond0','cond1','exp']
                    ).sample(
                            n=NIGHT_RESAMPLE,
                            replace=True
        ...
        cat_cols = ['cond1','cond0','ztime']
        for (this_cond1, this_cond0, this_ztime), group in IBI_data_night_resampled.groupby(cat_cols):
            expNum = group['expNum'].max()
            index_matrix = jackknife_resampling(np.array(list(range(expNum+1))))
            for excluded_exp, idx_group in enumerate(index_matrix):
                this_std = group.loc[group['expNum'].isin(idx_group),['IBI_yvel']].std().to_frame(name='IBIpitch_std')
                this_mean = group.loc[group['expNum'].isin(idx_group),['IBI_yvel']].mean()
    ...
    IBI_std_cond = IBI_data_cond.groupby(['ztime','cond0','cond1','exp','expNum']).std().reset_index()
    IBI_std_cond = IBI_std_cond.assign(IBIpitch_std = IBI_std_cond['IBI_yvel'])
# IBI_std_day_resampled = IBI_data_day_resampled.groupby(['ztime','cond0','cond1','expNum']).std().reset_index()
...
# plot kde of all
g = sns.FacetGrid(IBI_data_cond, 
                  row="ztime", row_order=all_ztime,
                  col='cond0', col_order=cond0,
                  hue='cond1', hue_order=cond1,
                  )
g.map(sns.kdeplot, "IBI_yvel",alpha=0.5,)
g.add_legend()
filename = os.path.join(fig_dir,"IBI_yvel_kde.pdf")
...
for feature in ['IBI_yvel_std']:
    ...
    filename = os.path.join(fig_dir,f"{prename}IBI_vel_std{x_name}__{gridcol}X{gridrow}.pdf")
    plt.savefig(filename,format='PDF')
    plt.show()
