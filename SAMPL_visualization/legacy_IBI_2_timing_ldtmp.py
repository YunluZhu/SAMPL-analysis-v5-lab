...
binned_angles['lighting'] = 'light'
resampled_y['lighting'] = 'light'

binned_angles.loc[binned_angles['cond1'] == 'dd', 'lighting'] = binned_angles.loc[binned_angles['cond1'] == 'dd', 'lighting'].replace({'light':'dark'})
binned_angles.loc[(binned_angles['cond1'] == 'ld') & (binned_angles['ztime'] == 'night'), 'lighting'] = binned_angles.loc[(binned_angles['cond1'] == 'ld') & (binned_angles['ztime'] == 'night'), 'lighting'].replace({'light':'dark'},)
resampled_y.loc[resampled_y['cond1'] == 'dd', 'lighting'] = resampled_y.loc[resampled_y['cond1'] == 'dd', 'lighting'].replace({'light':'dark'},)
resampled_y.loc[(resampled_y['cond1'] == 'ld') & (resampled_y['ztime'] == 'night'), 'lighting'] = resampled_y.loc[(resampled_y['cond1'] == 'ld') & (resampled_y['ztime'] == 'night'), 'lighting'].replace({'light':'dark'},)


#%%

lighting_df = pd.concat([binned_angles.query("cond1=='ld' and ztime=='day'"),
                         binned_angles.query("cond1=='dd' and ztime=='day'"),
                         binned_angles.query("cond1=='ll' and ztime=='night'"),
                         binned_angles.query("cond1=='ld' and ztime=='night'")], ignore_index=False)
circadian_df = pd.concat([binned_angles.query("cond1=='dd'"),
                         binned_angles.query("cond1=='ll'"),], ignore_index=False)

lighting_y = pd.concat([resampled_y.query("cond1=='ld' and ztime=='day'"),
                         resampled_y.query("cond1=='dd' and ztime=='day'"),
                         resampled_y.query("cond1=='ll' and ztime=='night'"),
                         resampled_y.query("cond1=='ld' and ztime=='night'")], ignore_index=False)
circadian_y = pd.concat([resampled_y.query("cond1=='dd'"),
                         resampled_y.query("cond1=='ll'"),], ignore_index=False)

lighting_data = (lighting_df, lighting_y)
circadian_data = (circadian_df, circadian_y)

#%%

for _seq, (_angle_data, _y_data)in enumerate([lighting_data, circadian_data]):

    lighting_order = _y_data.lighting.unique()
    ztime_order = _y_data.ztime.unique()
    
    if _seq == 0:
        col=None
        hue='lighting'
        row='ztime'
        row_order = ztime_order
        
    elif _seq == 1:
        col=None
        hue='ztime'
        row='lighting'   
        row_order = lighting_order
    g = sns.relplot(x='IBI pitch',y='bout frequency', data=_y_data, 
                    kind='line',
    ...
    for i , g_row in enumerate(g.axes):
        for j, ax in enumerate(g_row):
            sns.scatterplot(data=_angle_data.loc[
                (_angle_data[row]==row_order[i]),:
                    ], 
