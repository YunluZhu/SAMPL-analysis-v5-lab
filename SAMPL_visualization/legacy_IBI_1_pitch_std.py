'''
plot median IBI body angle distribution and standard deviation.
...
pick_data = 'gfp_ctrl' # name of your dataset to plot as defined in function get_data_dir()
...
if if_jackknife:
    if which_ztime != 'night':
        ...
        for (this_cond1, this_cond0, this_ztime), group in IBI_angles_day_resampled.groupby(cat_cols):
            ...
            for excluded_exp, idx_group in enumerate(index_matrix):
                ...
                this_median = group.loc[group['expNum'].isin(idx_group),['IBI_pitch']].median()
                resampled_day_std = pd.concat([resampled_day_std, this_std.assign(cond0=this_cond0,
                                                                        cond1=this_cond1,
                                                                        expNum=excluded_exp,
                                                                        ztime=this_ztime,
                                                                        median_val=this_median)])
    ...
    IBI_std_cond = IBI_std_cond.assign(median_val = IBI_angles_cond.groupby(['ztime','cond0','cond1','exp','expNum'])['IBI_pitch'].median().values)
...
IBI_std_cond = IBI_std_cond.assign(
    median1sd = IBI_std_cond['median_val'] + 2* IBI_std_cond['IBIpitch_std']
)
