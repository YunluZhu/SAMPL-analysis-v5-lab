...
grab_fish_angle_ver = 'v5.3.20230816'
...
# %%
    ...
    print(f"{folder}: total bouts aligned = {total_bouts_aligned}")
...
def runMP(dlm_input):
    with Pool() as pool:
        for _ in tqdm.tqdm(pool.istarmap(run, dlm_input), total=len(dlm_input)):
            pass
        # pool.starmap(run, dlm_input)