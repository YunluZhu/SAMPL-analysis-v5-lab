...
from preprocessing.analyze_dlm_v3 import analyze_dlm_resliced
# from vf_visualization.boutV3_kinetics import FRAME_RATE
global program_version
program_version = 'V3.220310'
# %%
# Define functions
def grp_by_epoch(df):
...
def grab_fish_angle_v3(analyzed, fish_length,sample_rate):
    '''
    ...
    BOUT_WINDOW_HALF = math.ceil(0.3 * SAMPLE_RATE)  # 12 frames for 40hz
    # SM_WINDOW = math.ceil((3/40) * SAMPLE_RATE)  # 3 for 40hz
    SM_WINDOW = 3
    SPD_SM_WINDOW = math.ceil((1/40) * SAMPLE_RATE) # if SAMPLE_RATE==40 else 3  # 1 for 40hz, smooth speed for bout detection
    
    # correct smooth windows, must be odd
    SM_WINDOW = SM_WINDOW-1+SM_WINDOW%2
    SPD_SM_WINDOW = SPD_SM_WINDOW-1+SPD_SM_WINDOW%2

    EDGE_CHOP = math.ceil((5/40) * SAMPLE_RATE)   # number of samples to remove from the beginning and end of each vector to account for edge effects (improper detection of fish body and movement)
    BOUT_LONG_TAIL = SAMPLE_RATE  # align prop bouts with longer duration (20 extra frames for 40hz)

    PRE_PEAK_FRAMES = math.ceil(SAMPLE_RATE * 0.75)  # s, Only align bouts with 30 extra frames before peak speed
    POST_PEAK_FRAMES = math.ceil(SAMPLE_RATE * 0.5)  # s, Only align bouts with 20 extra frames after peak speed
    ...
    frame_number25 = math.ceil(0.025*SAMPLE_RATE)
    frame_number100 = math.ceil(0.1*SAMPLE_RATE) # 100ms, 4 at 40Hz
    frame_number250 = math.ceil(0.25*SAMPLE_RATE) # 250ms, 10 at 40Hz
    frame_number125 = math.ceil(0.125*SAMPLE_RATE) # 125ms, 5 at 40Hz
    ...
    # )
    spd_window = spd_window.assign(
        # ifSwim = smooth_series_ML(spd_window['swimSpeed'],SPD_SM_WINDOW)>= speed_threshold
        ifSwim = smooth_series_ML(spd_window['swimSpeed'],1)>= speed_threshold
    )
    ...
    #  Here, the fast_win_ed is the last frame above threshold. Therefore, to get the same swim delay, one extra frame is added.
...
def run(filenames, folder, frame_rate):
    ...
    for i, file in enumerate(filenames):
        ...
        res = grab_fish_angle_v3(analyzed, fish_length,frame_rate)
        ...
        total_bouts_aligned = total_bouts_aligned + len(res['prop_bout2'])
...
# %%
    frame_rate_ln = f"Frame Rate: {frame_rate} \n"
    fish_len_ln = f"Estimated Fish Length Mean: {fish_length.mean()[1]} \n"
    fish_len_std_ln = f"Estimated Fish Length Std: {fish_length.std()[1]} \n"
    total_bouts_ln = f"Total Bouts Aligned: {total_bouts_aligned} \n"
    ana_ver_ln = f"Program Version: {program_version} \n"
    # box info
    
    with open(f'{output_dir}/analysis info.txt', 'w') as f:
        f.writelines([frame_rate_ln, 
                      fish_len_ln, 
                      fish_len_std_ln, 
                      total_bouts_ln,
                      ana_ver_ln])